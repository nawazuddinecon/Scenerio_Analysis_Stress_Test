...
for (name in names(shock_2013)) {
  idx <- which(colnames(ts_data_stat) == name)
...
for (vname in bank_vars) {
  var_idx <- which(colnames(ts_data_stat) == vname)
...
for (i in seq_along(bank_vars)) {
  var_idx <- which(colnames(ts_data_stat) == bank_vars[i])
...
for (vname in bank_vars) {
  means <- sapply(horizons, function(h) mean(forecast_mean[h, which(colnames(ts_data_stat) == vname)]))
...
summary_years
str(summary_years)
write.csv(summary_years, "shock_realization_probabilities_2022.csv", row.names = FALSE)
